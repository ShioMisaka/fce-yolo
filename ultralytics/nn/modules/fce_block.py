...
class BiCoordCrossAtt(nn.Module):
    ...
    广播相加，再过单次 Sigmoid 得到完整 [H, W] 门控信号。 旧版「双 Sigmoid 相乘」会把主干信号压制到 ≤25%（两个 0.5 相乘）， 且 weight_h[H,1]*weight_w[1,W] 是秩-1
    矩阵、表达力弱，是 5 个尺度 里 4 个拉低 mAP 的元凶。改用加性融合后信号保留率与标准 Coordinate Attention 一致（~50%），且 gate=gate_h+gate_w 让行列权重独立可学。
