...
def _collect_modules(model) -> dict:
    ...
    from ultralytics.nn.modules.fce_block import BiCoordCrossAtt, BiFPN_Concat
...
def inspect_bicoord(name: str, mod) -> list:
    ...
    仅看静态权重不够，还需看前向门控值的分布。但前向需要输入，这里先报静态权重 统计；若想看门控动态值，用 --input-shape 触发前向探测。
    """
...
def probe_gate_values(mod, input_shape: tuple) -> list:
    ...
    lines = ["    --- 门控值前向探测 ---"]
    mod.eval()
    n, _c = input_shape[0], input_shape[1]
    ...
    if hasattr(mod, "pool_h"):  # 确认是 BiCoordCrossAtt
        ...
        lines.append(
            f"      gate 值域: [{gate.min().item():.4f}, {gate.max().item():.4f}] "
...
def inspect_model(weight_path: Path, input_shape: tuple | None = None) -> None:
